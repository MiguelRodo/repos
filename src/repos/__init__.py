...
def main():
    """CLI entry point for the ``repos`` command installed by pip."""
...
"""
    ...
    if args[0] in ("--version", "-V"):
        print(_BUNDLED_CLI_VERSION)
        sys.exit(0)

    subcommand = args[0]
    rest = args[1:]

    if subcommand not in _SCRIPT_MAP:
        sys.stderr.write(f"Error: unknown command '{subcommand}'\n\n")
        sys.stderr.write(_USAGE)
        sys.exit(1)

    try:
        script_path = get_script_path(_SCRIPT_MAP[subcommand])
        os.chmod(script_path, 0o755)
        result = subprocess.run([script_path] + rest)
