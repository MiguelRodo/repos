...
validate_dir_name() {
  ...
  if [ -n "$dir" ]; then
    case "$dir" in
      /*|*..*)
        printf "Error: directory name cannot be absolute or contain '..': %s\n" "$dir" >&2
        return 1
        ;;
    esac
  fi
...
validate_script_path() {
  ...
  if [ -n "$script_path" ]; then
    ...
    case "$script_path" in
      /*|*..*)
        printf "Error: script path cannot be absolute or contain '..': %s\n" "$script_path" >&2
        return 1
        ;;
    esac
      ...
      case "$folder_path" in
        /*|..|*/..|../*|*/../*)
          ...
          if [ "$is_valid" = false ]; then
            printf "Error: invalid workspace folder path (unauthorized '..' or absolute): %s\n" "$folder_path" >&2
