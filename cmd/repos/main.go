...
func (s *state) cloneOneRepo(ins instruction) (int, error) {
	...
	if ref != "" {
		if _, err := gitcmd.RunGit("", "ls-remote", "--exit-code", "--heads", "--", repoURL, ref); err == nil {
			...
			if !s.allowCreate {
				return 1, fmt.Errorf("remote branch '%s' not found on %s (rerun with --create to create it)", ref, remoteHTTPS)
			}
...
func (s *state) createWorktreeForBranch(base, branch, targetDir, fetchMode string) (int, error) {
	...
	if !s.allowCreate {
		return 1, fmt.Errorf("branch '%s' not found on remote origin (rerun with --create to create it)", branch)
	}
